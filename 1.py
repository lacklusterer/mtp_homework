...
def cipher_xor(c1, c2):
    c1 = bytes.fromhex(c1)
    c2 = bytes.fromhex(c2)
    xored = list(map(lambda x, y: x^y, c1, c2))
    return bytes(xored)

def score_space(block:list):
    score = 0
    for i in range(len(block)):
        if 65<=block[i]<=90 or 97<=block[i]<=122:
            score += 1
    return score
...
for i in range(len(c)):
    if isinstance(key[i], bytes):
        print(chr(int.from_bytes(key[i],byteorder='big') ^ c[i]), end="")
    else:
        print("_", end="")